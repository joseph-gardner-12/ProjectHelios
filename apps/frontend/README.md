...
in `src/index.css`. The control page contains the interactive drone simulation.
...
Local routing uses Vite's SPA fallback at `http://projecthelios.localhost`.
Production uses `https://projecthelios.dev`, currently redirected to the `www`
hostname. Set Vercel's Root Directory to `apps/frontend`; `vercel.json` defines
the build and frontend rewrites so direct links and refreshes work. Add new
frontend routes to its rewrite list. `/api` requires separate production backend
routing and never uses the local Django proxy in production.
